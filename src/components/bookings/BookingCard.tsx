...
const BookingCard: React.FC<BookingCardProps> = ({
  ...
  return (
    <>
      <div className={`card border w-full ${isActive ? 'border-teal-300 bg-teal-50' : 'border-gray-200'} ${variant === 'list' ? 'p-2' : 'p-3'}`}>
...
<>
  ...
  <button onClick={handleCheckIn} className="btn btn-primary w-40">Check In</button>
  ...
  {(userRole === 'admin' || userRole === 'manager') && (
    <button onClick={handleCancellation} className="btn btn-danger w-40" disabled={!!cancellationRequest?.status === 'pending'}>
      ...
      {booking.checkInDateTime && !booking.checkOutDateTime && (
        <>
          <button onClick={() => setShowExtendModal(true)} className="btn btn-secondary w-40">Extend</button>
          <button onClick={handleCheckOut} className="btn btn-primary bg-blue-600 w-40">Check Out</button>
        </>
      )}
